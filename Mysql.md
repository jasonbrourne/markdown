...
![执行流程图](E:\markdown\images\mysqlExecutingProcess.png)
...
1. **写入undo log**：innodb 引擎首先开启事务，获得一个事务ID(是一直递增的)，根据执行的语句生成一个反向的语句，(如果是INSERT会生成一条DELETE语句，如果UPDATE语句就会生成一个UPDATE成旧数据的语句)，用于提交失败后回滚，将这条反向语句写入undo log，得到回滚指针，并且更新这个数据行的回滚指针和事务id。

2. **查找数据**：如果在内存中直接返回，如果不在内存中，查询后返回。

3. **更新数据**：修改数据并写入数据到内存中。

4. **写redo log**：将对数据页的更改写入到redo log，此时redo log中这条事务的状态为prepare状态。

5. **写bin log**：将更新的SQL写入bin log，然后调用innodb引擎接口将redo log设置为提交状态，更新完成。
