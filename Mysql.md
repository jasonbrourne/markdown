...
* **共享锁（Shared Lock/S锁/读锁）**
  ...
  * 定义：允许事务读取数据，阻止其他事务获取相同数据集的排他锁。多个事务可同时对同一数据加共享锁，实现并发读取‌。
  
  * 加锁方式：
    
    ```sql
    // 自动加锁：在 ‌Serializable‌ 隔离级别下，‌所有 SELECT 查询默认隐式加共享锁‌
    // 手动加锁：
    LOCK TABLE table_name READ; // 方式1,表加共享锁
    SELECT * FROM table_name LOCK IN SHARE MODE; // 方式2,行加共享锁
    SELECT * FROM table_name FOR SHARE; // 方式3,行加共享锁
    ```
  
  * 特性：允许其他事务获取读锁，阻塞其他事务获取写锁。

* **排他锁（Exclusive Lock/X锁/写锁）**
  
  * 定义：允许事务修改数据，阻止其他事务对同一数据集加任何类型的锁（包括共享锁和排他锁）。
  
  * 加锁方式：
    
    ```sql
    // 自动加锁：执行 UPDATE、DELETE、INSERT 语句时默认加排他锁‌
    // 手动加锁：
    LOCK TABLE table_name WRITE; // 方式1,表加排他锁
    SELECT * FROM table_name FOR UPDATE; // 方式2,行加排他锁
    ```
  
  * 特性：阻塞其他事务的所有锁请求，包括共享锁和排它锁。

**粒度性分类**

* **全局锁**
  
  全局锁就是对整个数据库实例加锁。MySQL 提供了一个加全局读锁的方法，命令是Flush tables with read lock (FTWRL)。执行完命令，数据库处于只读状态。执行DDL、DML等语句会被阻塞。

* **表级锁**
  
  表级锁对整张表加锁，MyISAM 与 InnoDB 都支持表级锁定。其中MyISAM只支持表锁，InnoDB在某些特定场景下使用表锁‌。
  
  **InnoDB使用表锁场景：**
  
  * 当执行 `UPDATE`、`DELETE` 或 `SELECT ... FOR UPDATE` 等操作时，若 ‌**未使用索引**‌ 或 ‌**索引失效**‌，InnoDB 会退化为 ‌**全表扫描**‌，此时行锁无法精准定位数据行，只能升级为 ‌**表级锁**
  
  * 执行 `ALTER TABLE`、`DROP TABLE`、`TRUNCATE TABLE` 等数据定义语言(DDL)‌ 时，InnoDB会对整个表加**表级锁‌** ，阻止其他事务的读写操作，确保结构变更的原子性‌。
  
  * 通过 `LOCK TABLES table_name [READ|WRITE]` 语句手动请求表锁，此时 InnoDB 会直接锁定整张表‌。
  
  * 若事务持有大量行锁且持续时间长，InnoDB 可能将锁升级为表锁以减少锁冲突和死锁风险‌。
  
  * **自动增长列（AUTO_INCREMENT）**：在高并发插入场景下，为维护自增列的唯一性，InnoDB 可能在插入时短暂使用表锁‌。
  
  **加锁方式：**
  
  ```sql
  // 自动加锁:
  // MyISAM在执行DQL语句时自动加读锁,在执行DML语句时自动加写锁
  // 所有引擎在执行DDL时都会加写锁
  // 手动加锁：
  LOCK TABLES table1 READ, table2 READ; // 加读锁
  LOCK TABLES table1 WRITE, table2 WRITE; // 加写锁
  UNLOCK TABLES; // 解锁
  ```
  
  表锁实现简单、开销小，死锁概率低，但是并发性能差。适用于并发度低，读多写少的场景。
  
  **元数据锁（meta data lock，MDL)：**
  
  MDL（Metadata Lock）是 MySQL 中用于保护表元数据一致性的锁机制，确保在对表进行结构变更（如 DDL 操作）时，不会与数据操作（如 DML 操作）产生冲突‌。MDL不需要显式使用，在访问一个表的时候会被自动加上。MDL的作用是，保证读写的正确性。
  
  MDL 锁分为两种类型，通过读写互斥规则控制并发：
  
  - **‌共享锁（S锁）**：当对一个表做增删改查操作的时候，加MDL读锁。读锁之间不互斥，因此可以有多个线程同时对一张表增删改查。
  - ‌**排他锁（X锁）**：当要对表做结构变更操作的时候，加MDL写锁，MDL写锁阻塞所有其他操作‌。读写锁之间、写锁之间是互斥的，用来保证变更表结构操作的安全性。因此，如果有两个线程要同时给一个表加字段，其中一个要等另一个执行完才能开始执行。**写锁请求优先于读锁，且一旦出现写锁等待，后续所有操作均被阻塞‌。**
  
  > Tips：MDL锁是系统自动添加，无法手动控制。由于执行DDL语句会给表加写锁，并且阻止后续一切的操作。如果业务中SQL频繁，就会导致DDL语句等待，从而进一步导致后续业务其他SQL阻塞，导致表完全不可读写。所以避免在业务高峰期执行DDL语句。另外MariaDB已经合并了AliSQL的DDL NOWAIT/WAIT 语法。支持DDL语句超时退出。
  > 
...
* **页级锁**
  ...
  页级锁是 MySQL 中锁定粒度介于行级锁和表级锁中间的一种锁，锁定对象为数据存储的物理页（通常包含多行数据）。表级锁速度快，但冲突多，行级冲突少，但速度慢。因此，采取了折衷的页级锁，一次锁定相邻的一组记录。BDB 引擎支持页级锁。
